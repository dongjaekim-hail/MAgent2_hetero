from model import DQN, ReplayBuffer_cen
import numpy as np
import random
import torch
...
class MADQN_cen():  # def __init__(self,  dim_act, observation_state):
    ...
    def replay(self): #배치사이즈 살려야 할 것 같은데....나중에 unsqueeze 없애야함
        for _ in range(10):
            ...
            q_values = self.gdqn(observations.unsqueeze(0))# 실제로 observation에서의 q 값
            q_values = q_values[0][actions]
            #아 adj 를 unsqueeze 를 해야하는것 같다. 그래서 36*36 을 1*36*36 으로 해주어야 densesageconv를 할 수 있는 것 같다.

            next_q_values = self.gdqn_target(next_observations.unsqueeze(0))  # next state에 대해서 target 값
