...
class MADQN():  # def __init__(self,  dim_act, observation_state):
    def __init__(self, n_predator1, n_predator2, predator1_obs, predator2_obs, dim_act ):
        ...
        self.gdqns = [G_DQN(self.dim_act, self.predator1_obs) for _ in range(1)] + [
            G_DQN(self.dim_act, self.predator2_obs) for _ in range(1)]
        self.gdqn_targets = [G_DQN(self.dim_act, self.predator1_obs) for _ in range(1)] + [
            G_DQN(self.dim_act, self.predator2_obs) for _ in range(1)]  # 학습의 안정을 위해 target dqn 설정
        print(len(self.gdqn_targets))
        print(self.gdqn_targets)
        self.buffers = [ReplayBuffer() for _ in range(2)]
    ...
    def get_action(self, state, mask=None):
        ...
        q_value = self.gdqn(state, self.adj ) #shared_info : shared graph에 넘겨주어야 할 정보들
    ...
    def replay(self): #배치사이즈 살려야 할 것 같은데....나중에 unsqueeze 없애야함
        for _ in range(10):
            ...
            q_values, _ = self.gdqn(observations.unsqueeze(0), (self.adj).unsqueeze(0))# 실제로 observation에서의 q 값
            q_values = q_values[0][actions]
#아 adj 를 unsqueeze 를 해야하는것 같다. 그래서 36*36 을 1*36*36 으로 해주어야 densesageconv를 할 수 있는 것 같다.

            next_q_values, _ = self.gdqn_target(next_observations.unsqueeze(0), (self.adj).unsqueeze(0))  # next state에 대해서 target 값
