...
for ep in range(1000):
    env.reset()
    print("ep:", ep)
    #매 에피소드별로 observation 과 next_observation, reward, terminat을 에이전트별로 저장하는 리스트를 만들고..
    #env.last() 해서 나오는 observation 값을 저장하고
    #env.step() 한다.
    #그리고 다음 iteration 에서 나오는 env.last() 값이 에이전트의 next_observation 의 값이다.
    ...
    for agent in env.agent_iter():
        ...
        if agent[:8] == "predator":  # predator 일때만 밑의 식이 실행되어야 함
            ...
            if termination or truncation:
                ...
                madqn.buffer.put(observation_temp, action, reward, process_array(next_observation), termination,
                                 truncation)
