from model2 import G_DQN, ReplayBuffer
import numpy as np
import random
import torch
...
class MADQN():  # def __init__(self,  dim_act, observation_state):
    ...
    def replay(self): #배치사이즈 살려야 할 것 같은데....나중에 unsqueeze 없애야함
        for _ in range(10):
            ...
            print(np.array(observations).shape,"observations.shape")
            next_observations = torch.tensor(next_observations)
            observations = torch.tensor(observations)

            next_observations = next_observations.reshape(args.batch_size,-1,3)
