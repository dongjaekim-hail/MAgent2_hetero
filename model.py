...
class G_DQN(nn.Module):
    ...
    def forward(self, x, adj, info): #x외 adj는 밖에서 넣어줘야 되고  GSAGE에 입력값 넣어주면 출력값 뱉고, from_guestbook 아예 크기에 맞는 (8*8*7)의 형태로 넣어주고
        ...
        #print("이거 확인해보자",a)
        #노드끼리 fully connective 되어 있다는 가정아래!gnn어차피 fully connective 여서 mask 인자 없애버림
        x = F.elu( self.gnn2(self.gnn1(x.reshape(-1, self.dim_feature) ,adj),adj)).squeeze()  # exponential linear unit #squeeze 를 하는 이유: x가 batch_size를 고려해서 받을 수 있도록 설계 됐기 때문에 1*100*14꼴로 나옴->100*14으로 바꿔주기 위함
