	...
	for ep in range(1000):
		...
		for agent in env.agent_iter():
			...
			if step_idx == 0:   #첫번째 step
				...
				print("iteration",(iteration_number // (n_predator1 + n_predator2 + n_prey)))
				...
				if agent[:8] == "predator":  # predator 일때만 밑의 식이 실행되어야 함
					...
					if termination or truncation:
						...
						madqn.buffer.put(observations_dict[idx][step_idx-1], action, reward_dict[idx][step_idx]-reward_dict[idx][step_idx-1],
