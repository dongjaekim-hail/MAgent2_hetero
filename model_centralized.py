...
class G_DQN(nn.Module):
    def __init__(self,  dim_act, observation_state):
        ...
        self.gnn2 = DenseSAGEConv(128, self.dim_feature) #feature 의 2배를 출력
        #self.gnn2 = DenseSAGEConv(128, self.dim_feature * 2)  # feature 의 2배를 출력
        self.sig = nn.Sigmoid() #sigmoid 도 이렇게 해야 하고..

        #DQN
        self.dim_input = observation_state[0] * observation_state[1] * observation_state[2] #concat 해서 2배!
    ...
    def forward(self, state, adj): #x외 adj는 밖에서 넣어줘야 되고  GSAGE에 입력값 넣어주면 출력값 뱉고, from_guestbook 아예 크기에 맞는 (8*8*7)의 형태로 넣어주고
        ...
        print("x의 차원 확인",x.shape)
        print("adj의 차원 확인", adj.shape)
        #print("이거 확인해보자",a)
        ...
        return x  #shared_graph에 넣는건 밖에서 진행하자.
